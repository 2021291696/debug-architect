...
| 产出目标 | 内容 | 执行方式 |
|---------|------|---------|
| 项目 `ERROR_LOG.md` | 错误描述 + 解法 + 预防措施（全量） | 自动写入 |
| 项目 `CLAUDE.md` | 预防规则推荐 | **展示推荐 → 用户确认 → 修改** |
| `memory/` | 跨会话操作记忆 | 按需判断（见下方判断表） |
| `wiki/开发踩坑录.md` | 方法论级洞察 | 按需判断（见下方判断表） |
...
| 产出目标 | 内容 | 执行方式 |
|---------|------|---------|
| 项目 `ERROR_LOG.md` | 错误描述 + 推测依据 + 验证方法 | 自动写入 |
| 验证选项 | "要验证这个推测吗？" | 展示给用户 |
